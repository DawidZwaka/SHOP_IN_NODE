...
userSchema.methods.addToCart = function (product) {
  ...
  if (cartProducts.length === 0) {
    ...
    cartProducts.find(
      (cartProd) => String(cartProd.productID) === String(product._id)
    )
  ) {
  ...
  return this.populate({
    ...
    .then((products) => {
      ...
      if (products.length === 0) total = 0;
        ...
        total = products[0].qty * products[0].price;
      else
        total = products.reduce(
          (prev, next) => prev.qty * prev.price + next.qty * next.price
        );
...
userSchema.methods.removeFromCart = function (productID) {
  const updatedCartProducts = this.cart.products.filter(
