...
const messages = {
  singup: {
    ...
    email: {
      ...
      emailTaken:
        "User with that email already exist, please choose another one.",
    },
    password: {
...
router.post(
  "/login",
  [
    body("email").isEmail().withMessage(messages.login.email.basic),
    body("password")
...
router.post(
  "/signup",
  [
    body("name", messages.singup.name.basic).isAlphanumeric(),
